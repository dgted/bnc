...
def Khola_kagoj_bd(start_date, end_date, path, listbox_selected_category):
    import bs4
    import requests
    import csv
    import pandas as pd
    ...
    #http://www.kholakagojbd.com/archiveq/2020-12-01
    base_address = "http://www.kholakagojbd.com/archiveq"
    ...
    base_address = "http://www.kholakagojbd.com/"
    ...
    def get_sub_links(main_page_link, path, listbox_selected_category):
        ...
        print(soup)
        # newsArticleDivs = soup.find_all("div", {"class": "col-sm-4 col-sm-5 col-sm-7 col-sm-8 n_row"})
        # newsArticleDivs = soup.find_all("div", {"class": "col-sm-5"})

        newsArticleDivs = soup.find_all("div", {"class": "category-lead-news-main-right"})
    ...
    CSV_LINK = 'kholakagojbd_entertainment_news_151to202_(1091).csv'

    def append_to_csv(row, path, category):
        global CSV_LINK
        if category == 'All':
            CSV_LINK = path + "/Khola-Kagoj-bd_" + category + "_news.csv"
        else:
            CSV_LINK = path + "/Khola-Kagoj-bd_" + category + "_news.csv"
    ...
    def get_title_and_content(url, news_date,  path, listbox_selected_category):
        ...
        #print(newsTitle)
        article_text = soup.find("div", {"class": "details-news-area"})

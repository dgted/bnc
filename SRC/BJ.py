def BJ(s, e, Path, listbox_selected_category):
    global path
    ...
    base_address = "https://www.bd-journal.com/archive"
    ...
    for d in All_date:
        complete_url = base_address + '/' + 'online-edition/' + str(d)
        print(complete_url)
        all_url.append([complete_url])
        pass
    ...
    def get_title_and_content(url, date, Now_categ, path):
        ...
        article_text = soup.find("div", {"class": "dtl_pg_row"})
        ...
        for i in listbox_selected_category:
            ...
            if string in url:
                ...
                append_to_csv(entry, "Bangladesh_journal_" + string, path)
                append_to_csv(entry, "Bangladesh_journal_all_category", path)

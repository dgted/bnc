...
def Manob_Kantha(start_date, end_date, path, listbox_selected_category):
    import bs4
    import requests
    import csv
    import pandas as pd
    ...
    #https://www.manobkantha.com.bd/archive/2020-12-01
    # https://www.kalerkantho.com/print-edition/first-page/2020/12/01
    base_address = "https://www.manobkantha.com.bd/archive"
    ...
    def get_sub_links(main_page_link, path, listbox_selected_category):
        ...
        for newsArticle in newsArticleDivs:
            for p in newsArticle.find_all('li'):
                a_tag = p.find("a")
    ...
    CSV_LINK = 'campus_dataset_new.csv'

    def append_to_csv(row, path, category):
        global CSV_LINK
        if category == 'All':
            CSV_LINK = path + "/Manob-Kantho_" + category + "_news.csv"
        else:
            CSV_LINK = path + "/Manob-Kantho_" + category + "_news.csv"
    ...
    def get_title_and_content(url, news_date, path, listbox_selected_category):
        ...
        #print(newsTitle)
        article_text = soup.find("div", {"class": "detail-article"})

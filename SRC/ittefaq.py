...
def itte_faq(start_date, end_date, path, listbox_selected_category):
    import bs4
    import requests
    import csv
    import pandas as pd
    ...
    base_address = "https://www.ittefaq.com.bd/archive/print-edition"
    ...
    CSV_LINK = 'datasetittefaq.csv'

    def append_to_csv(row, path, category):
        global CSV_LINK
        if category == 'All':
            CSV_LINK = path + "/Ittefaq_" + category + "_news.csv"
        else:
            CSV_LINK = path + "/Ittefaq_" + category + "_news.csv"
    ...
    def get_title_and_content(url, path, listbox_selected_category):
        ...
        # print(newsTitle)post_date_time
        date_text = soup.find("div", {"class": "post_date_time"})
        all_dateinfo = date_text("span")

        article_text = soup.find("div", {"class": "dtl_content_block"})
        ...
        ss = 0
        for paragraph1 in all_dateinfo:
            ss = ss + 1
            if (ss == 2):
                news_content1 += paragraph1.getText()

        if news_content1 == '':
            news_content1 = date_text.contents[-1].strip()
        ...
        for cat in listbox_selected_category:
            ...
            if cat in url:
                cat_all = cat
                input_array = [date, newsTitle, news_content, cat]
                append_to_csv(input_array, path, cat)


        #date = news_content1
        #input_array = [newsTitle, news_content, news_content1, type]
        input_array = [date, newsTitle, news_content, cat_all]
